...
jQuery(function ($) {
  ...
  $.widget("flowchart.flowchart", {
             ...
             <div class="list-group-item-button" style="color: inherit">
              ...
              <div class="d-flex list-group-item-controls">
                <form enctype="multipart/form-data" method="POST" target="hiddenframe" action="/api/linkRemove?linkId=${el._id}" class="d-flex flex-column justify-content-center py-2">
                  ...
                  <label class="btn btn-danger m-0 list-group-item-delete"  data-toggle="tooltip"  title="Delete this link" onclick="$('#chart').flowchart('redrawLinksLayer'); this.closest('form').remove()">
    ...
    _autoCreateSubConnector: function (operator, connector, connectorType, subConnector) {
      var connectorInfos = Object.values(this.data.operators).find(x => x.id == operator).internal.properties[connectorType][connector];
      if (connectorInfos.multiple) {
        var fromFullElement = Object.values(this.data.operators).find(x => x.id == operator).internal.els;
        var nbFromConnectors = Object.values(this.data.operators).find(x => x.id == operator).internal.els.connectors[connector].length;
    ...
    getConnectorPosition: function (operatorId, connectorId, subConnector) {
      ...
      var operatorData = Object.values(this.data.operators).find(x => x.id == operatorId)
    ...
    _drawLink: function (linkId) {
      ...
      var fromOperator = Object.values(this.data.operators).find(x => x.id == fromOperatorId);
      var toOperator = Object.values(this.data.operators).find(x => x.id == toOperatorId);
    ...
    _getOperatorFullElement: function (operatorData) {
      ...
      var $operator_title_span = $('<div class="flowchart-operator-title-span"></div>');
      $operator_title_span.html(infos.title);
      $operator_title.html($operator_title_span)
    ...
    createOperator: function (operatorId, operatorData) {
      ...
      fullElement.operator.css({
        top: function () {
          if (operatorData.top) {
            return (document.querySelector('.flowchart-container').offsetHeight / 3) + operatorData.top
          } else {
            return (document.querySelector('.flowchart-container').offsetHeight / 3)
          }
        },
        left: function () {
          if (operatorData.left) {
            return (document.querySelector('.flowchart-container').offsetWidth / 2) + operatorData.left
          } else {
            return (document.querySelector('.flowchart-container').offsetWidth / 2)
          }
        },
      });
    ...
    getBoundingOperatorRect: function (operatorId) {
      if (!Object.values(this.data.operators).find(x => x.id == operatorId)) {
        return null;
      }

      var elOperator = Object.values(this.data.operators).find(x => x.id == operatorId).internal.els.operator;
      var operator = Object.values(this.data.operators).find(x => x.id == operatorId);
    ...
    doesOperatorExists: function (operatorId) {
      return typeof Object.values(this.data.operators).find(x => x.id == operatorId) != 'undefined';
    },
